...
const BG_NUM = ['img/bg-crystalcave.png', 'img/bg-moonlit.png', 'img/bg-woodland.png'];
...
function renderDefeatModal() {
    // ... (keep existing renderDefeatModal code)
}
...
// ADD THIS: Enter key handler function
function battleEnterHandler(e) {
    if (e.key === 'Enter') {
        const quizArea = document.getElementById('quizArea');
        // Only submit if quiz is visible (not hidden)
        if (quizArea && !quizArea.classList.contains('d-none')) {
            e.preventDefault();
            
            // Determine which level's check function to call
            if (gameState.currentLevel === 1) {
                // Check if this is a boss battle (site 5)
                const isBoss = gameState.currentSite === 5;
                checkBattleAnswer(isBoss);
            } else if (gameState.currentLevel === 2) {
                const isBoss = gameState.currentSite === 5;
                if (typeof checkBattleAnswer_L2 === 'function') {
                    checkBattleAnswer_L2(isBoss);
                }
            } else if (gameState.currentLevel === 3) {
                const isBoss = gameState.currentSite === 5;
                if (typeof checkBattleAnswer_L3 === 'function') {
                    checkBattleAnswer_L3(isBoss);
                }
            }
        }
    }
...
// ADD THIS: Clean up enter key listener when battle ends
function showBattleResult(won, isBoss) {
